...
const Students = () => {
  ...
  return (
    <>
      <StudentsProfile isVisible={detailsShow} visiblitySet={setDetailsShow} details={details} setDetails={setDetails} />
      <My showprofile={profileVisible} setProfile={setProfile} />
      <Navbar title={"Students"} />
      <Navigations page={"Students"} showprofile={profileVisible} setProfile={setProfile} />
                      ...
                      data.map((ele, i) => {
                        return <tr key={i}>
                          ...
                          <td className="px-12 py-4 text-sm font-medium whitespace-nowrap">
                            <div className={`inline px-6 py-1.5 text-[12px] font-normal rounded-full gap-x-2 ${(ele.status==='Waiting'&& 'bg-amber-300') || (ele.status==='Present'&& 'bg-green-400') || (ele.status==='Absent'&& 'bg-red-400') }`}>
                              {ele.status}
                            </div>
                          </td>
                          <td className="px-4 py-4 text-sm whitespace-nowrap">
                            <div>
                              <h4 className="text-gray-700 dark:text-gray-200">Grade: {ele.grade}</h4>
                              <p className="text-gray-500 dark:text-gray-400">{ele.address}</p>
                            </div>
                          </td>
                          <td className="px-4 py-4 text-sm whitespace-nowrap">
